...
export default function Sidebar({ player, handleLogout }) {
  ...
  const menuItems = [
    ...
    {
      name: "Badges",
      href: player ? `/profile/${player.user_id}?tab=badges` : "#",
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
  ...
  return (
    <aside className="w-64 bg-[#05030a] border-r border-white/5 flex flex-col justify-between shrink-0 h-screen sticky top-0 z-40 p-6">
      <div className="flex flex-col gap-8">
        {/* ARENA LOGO */}
        <Link href="/" className="flex items-center justify-center group mt-2 w-full">
        ...
        <nav className="flex flex-col gap-1.5">
          {menuItems.map((item) => {
            const isActive = item.name === "Badges"
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="bg-[#110e20]/80 border border-white/5 rounded-2xl p-4 backdrop-blur-md relative overflow-hidden group">
          ...
          <span className="text-[9px] font-black uppercase tracking-wider text-slate-500">uPoints Balance</span>
          ...
          <Link href="/leaderboard" className="inline-flex items-center gap-1 text-[9px] font-bold text-slate-400 hover:text-white mt-3.5 transition-colors">
        ...
        <div className="flex flex-col gap-4 pt-4 border-t border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col min-w-0">
              <span className="text-[11px] font-black text-white truncate">{player?.name || "Arena Player"}</span>
